...
def create_session_for_users(db: Session, patient_id: int, therapist_id: int, session: schemas.Session):
    ...
    db_session = models.Session(patient_id=patient.id, therapist_id=therapist.id)
    db.add(db_session)
    db.commit()
